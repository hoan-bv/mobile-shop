...
<body>
	<?php 
	if(isset($_POST['sbm'])){
			//header(location:đến trang chủ):điều hướng đến trang chủ
		$username= $_POST['mail'];
		$userpass= $_POST['pass'];
		$query = mysqli_query($connect,"SELECT user_mail, user_pass FROM user WHERE user_mail='$username' AND user_pass='$userpass'");
    if (mysqli_num_rows($query) > 0) {
    	$_SESSION['tai_khoan']= $username;
    	$_SESSION['mat_khau']=$userpass;
